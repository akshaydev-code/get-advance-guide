...
import { Conversation, ChatMessage, MentorItem } from '../types';
...
interface MessagesTabProps {
  ...
  onStartVideoCall?: (mentorName: string) => void;
}
...
export default function MessagesTab({
  ...
  const [selectedConvId, setSelectedConvId] = useState<string>(
    activeMentorId
      ? conversations.find((c) => c.mentorId === activeMentorId)?.id || conversations[0]?.id || ''
      : conversations[0]?.id || ''
  );
  ...
  // Sync if activeMentorId changes externally
  useEffect(() => {
    if (activeMentorId) {
      const match = conversations.find((c) => c.mentorId === activeMentorId);
      if (match) {
        setSelectedConvId(match.id);
      }
    }
  }, [activeMentorId, conversations]);
  ...
  const handleSend = (e?: React.FormEvent) => {
    ...
    // Simulate mentor typing and reply after 1.5s
    setIsTyping(true);
    setTimeout(() => {
      setIsTyping(false);
    }, 2000);
  ...
  const filteredConversations = conversations.filter((c) =>
    ...
    c.mentorRole.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const quickPrompts = [
    'Can we reschedule our upcoming session?',
    'Here is my project GitHub repository link',
    'Thank you so much for the insightful guidance!',
    'Could you please share the design system slides?',
  ];
      ...
      <div className="w-full md:w-80 lg:w-96 border-r border-gray-100 flex flex-col bg-gray-50/40">
        ...
        <div className="p-4 border-b border-gray-100 bg-white">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-extrabold text-gray-900 text-base">Chats</h3>
      ...
      {activeConversation ? (
        <div className="flex-grow flex flex-col h-full bg-white">
          ...
          <div className="p-4 px-6 border-b border-gray-100 flex items-center justify-between bg-white z-10">
            <div className="flex items-center gap-3">
              <div className="relative">
                <img
                  src={activeConversation.mentorAvatar}
              ...
              <div>
                <h3 className="text-sm font-extrabold text-gray-900 flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-violet-700 bg-violet-50 px-2 py-0.5 rounded-full">
                    {activeConversation.mentorCompany}
                  </span>
                </h3>
                <p className="text-[11px] text-gray-500 font-medium">
                  {activeConversation.isOnline ? 'Online now • Replies quickly' : 'Away • Available tomorrow'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => onStartVideoCall && onStartVideoCall(activeConversation.mentorName)}
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-4 bg-gray-50/30">
            ...
            {isTyping && (
              <div className="flex items-center gap-2 text-xs text-gray-400 italic">
                ...
                />
                <span>{activeConversation.mentorName.split(' ')[0]} is typing...</span>
              </div>
            )}
          ...
          <form onSubmit={handleSend} className="p-4 px-6 bg-white border-t border-gray-100 flex items-center gap-3">
            <button
              type="button"
              onClick={() => alert('Attachments: PDF / Screenshot uploader simulated.')}
            ...
            <input
              type="text"
              placeholder={`Write a message to ${activeConversation.mentorName.split(' ')[0]}...`}
              value={inputText}
        ...
        <div className="flex-grow flex items-center justify-center p-12 text-center text-gray-400">
          <div>
            ...
            <p className="text-xs text-gray-500">Pick a mentor from the sidebar to start chatting.</p>
          </div>
        </div>
      )}
    </div>
