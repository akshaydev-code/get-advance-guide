...
import { X, Video, Mic, MicOff, VideoOff, MessageSquare, Share2, PhoneOff, Users, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function LiveMeetModal({
  ...
  const [chatMessages, setChatMessages] = useState<{ sender: string; text: string; time: string }[]>([
    { sender: 'Student', text: 'Hello Anubhav sir! I am ready with my architecture diagrams.', time: 'Just now' },
  ]);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setChatMessages((prev) => [
      ...prev,
      { sender: 'You (Mentor)', text: chatInput, time: 'Just now' },
    ]);
    setChatInput('');
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">
                Mentee: <strong className="text-gray-200">{session.studentName}</strong> ({session.studentRole})
              </p>
            </div>
          </div>
          ...
          <div className={`${meetingChatOpen ? 'md:col-span-8' : 'md:col-span-12'} h-full flex flex-col gap-3 transition-all duration-300`}>
            <div className="relative flex-grow bg-[#151722] rounded-2xl overflow-hidden border border-gray-800 flex items-center justify-center">
              ...
              <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 text-white flex items-center gap-2">
                ...
                <span className="text-xs font-bold">{session.studentName} (Mentee)</span>
              </div>

              {/* Self Video (Mentor Picture-in-Picture) */}
          ...
          {meetingChatOpen && (
            <div className="md:col-span-4 h-full bg-[#181a26] rounded-2xl border border-gray-800 flex flex-col overflow-hidden">
              <div className="p-3 bg-[#202336] border-b border-gray-800 text-xs font-bold text-gray-200 flex items-center justify-between">
                <span>Call Chat</span>
                <button onClick={() => setMeetingChatOpen(false)} className="text-gray-400 hover:text-white cursor-pointer">
                  <X size={14} />
                </button>
              </div>
              ...
              <div className="flex-grow p-3 overflow-y-auto space-y-3">
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`text-xs ${msg.sender.startsWith('You') ? 'text-right' : 'text-left'}`}>
              ...
              <form onSubmit={handleSendMessage} className="p-2 border-t border-gray-800 flex gap-2">
                ...
                <button
                  type="submit"
                  className="bg-violet-600 hover:bg-violet-700 text-white px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer"
                >
                  Send
                </button>
              </form>
