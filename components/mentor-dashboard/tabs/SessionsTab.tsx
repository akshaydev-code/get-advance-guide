...
import { MentorSession, MentorDashboardTab } from '../types';
...
export default function SessionsTab({
  ...
  const createGoogleCalendarUrl = (session: MentorSession) => {
    const title = encodeURIComponent(`1-on-1 Mentorship: ${session.topic} with ${session.studentName}`);
    const details = encodeURIComponent(`Mentorship Session on GetAdvanceGuide. Mentee: ${session.studentName}. Topic: ${session.topic}`);
    const location = encodeURIComponent('GetAdvanceGuide Live Room');
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">
              Mentorship Schedule & Live Calls
            </h2>
          ...
          <p className="text-xs text-gray-500 font-medium mt-1">
            Host live 1-on-1 video rooms, manage your teaching calendar, and review student feedback notes.
          </p>
        </div>
      </div>
      ...
      {filteredSessions.length === 0 ? (
        <div className="bg-white rounded-[2rem] p-12 text-center border border-gray-100 shadow-sm space-y-3">
          ...
          <p className="text-xs text-gray-500 max-w-sm mx-auto">
            When students book calls or you accept pending requests, sessions appear here.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-start sm:items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-extrabold text-gray-900">{session.studentName}</h3>
                      <span className="text-xs text-violet-600 font-bold">@{session.studentRole}</span>
                    </div>
              ...
              {session.studentFeedback && (
                <div className="p-3 bg-amber-50/70 rounded-xl border border-amber-200 text-xs text-amber-900">
                  <strong>Mentee Feedback:</strong> ★ {session.studentRating}/5 — <em>&ldquo;{session.studentFeedback}&rdquo;</em>
                </div>
              )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-gray-100">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => onStartMessage(session.studentId)}
                  ...
                  >
                    <MessageSquare size={13} /> Chat with {session.studentName.split(' ')[0]}
                  </button>
                ...
                <div className="flex items-center gap-2">
                  {session.status === 'Upcoming' && (
                    <>
                      ...
                      <button
                        onClick={() => {
                          if (confirm(`Are you sure you want to cancel the session with ${session.studentName}?`)) {
